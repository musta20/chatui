...
const { SELECT_TICKET, CLOSE_TICKET , SHOW_TICKETS ,SHOW_MY_TICKETS , inferToObjectId} = require("../utils/type");

module.exports  = (socket) => {

  socket.on(SELECT_TICKET, async ( ticket , callback) => {
    
    const ticketfind = await TicketModel.findOne({ _id:inferToObjectId( ticket.id )});
  ...
  socket.on(CLOSE_TICKET, async (ticket , callback) => {
    ...
    const ticketfind = await TicketModel.findOne({ _id:inferToObjectId( ticket.id ),SupportId:ticket.myid});
